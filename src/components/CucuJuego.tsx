              ...
              {roundSpots.map((spot, i) => {
                ...
                return (
                  <motion.button key={`${target.code}-${i}`} onClick={() => handleTap(i)}
                    ...
                    {isCorrectSpot && (
                      <div className="absolute inset-0 flex items-start justify-end overflow-hidden p-1">
                        <img src={`${FLAG_BASE}/${target.code.toLowerCase()}.svg`} alt="" onError={handleFlagError}
                          className="w-1/3 h-1/3 object-contain -mr-2 -mt-2 opacity-80" draggable={false} />
                      </div>
                    )}
          ...
          {target && revealed && (
            <motion.div key="revealed" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 150, damping: 12 }}
              ...
              <motion.div initial={{ y: -200, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}
                ...
                {roundSpots.map((spot, i) => (
                  <motion.span key={i}
                    animate={i === 0 && flagFlying ? { y: -300, opacity: 0, rotate: 30 } : {}}
              ...
              <motion.img initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }}
                ...
                src={`${FLAG_BASE}/${target.code.toLowerCase()}.svg`} alt="" onError={handleFlagError}
