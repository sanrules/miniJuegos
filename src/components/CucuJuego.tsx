...
export function CucuJuego({ poolCountries, onBack, onFinish }: GameProps) {
  ...
  const generateRound = useCallback(() => {
    ...
    if (!t) return;
    setTarget(t);
    setCorrectIndex(Math.floor(Math.random() * spots.length));
    setRevealed(false);
  ...
  const handleTap = useCallback((index: number) => {
    ...
    if (index === correctIndex) {
      ...
      setTimeout(() => speak(`🎉 ¡Cucú! ¡${target.name}!`), 500);
      setTimeout(() => { generateRound(); }, 3000);
    } else {
      setShakingIndex(index);
      setSurpriseEmoji(surprises[Math.floor(Math.random() * surprises.length)]);
      setTimeout(() => { setShakingIndex(null); setSurpriseEmoji(null); }, 800);
    }
  }, [revealed, target, correctIndex, speak, generateRound]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 py-6 px-4">
      ...
      <main className="max-w-lg mx-auto min-h-[500px] flex items-center justify-center">
        <AnimatePresence mode="wait">
          {target && !revealed && (
            <motion.div key="hiding" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              {spots.map((spot, i) => {
                ...
                const isCorrectSpot = i === correctIndex;
                return (
                  <motion.button key={i} onClick={() => handleTap(i)}
                    animate={isShaking ? { x: [0, -12, 12, -8, 8, 0], rotate: [0, -8, 8, -5, 5, 0] } : {}}
                    ...
                    {isCorrectSpot && (
                      <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
                        <img src={`${FLAG_BASE}/${target.code.toLowerCase()}.svg`} alt=""
                          className="absolute w-3/4 h-3/4 object-contain opacity-60 rotate-12 translate-x-3 -translate-y-2" draggable={false} />
                      </div>
                    )}
                    <span className="text-6xl md:text-7xl relative z-10">{spot.emoji}</span>
                    <AnimatePresence>
                      {isShaking && surpriseEmoji && (
                        <motion.span key="s" initial={{ scale: 0, y: 10 }} animate={{ scale: 1, y: -15 }} exit={{ scale: 0 }}
          ...
          {target && revealed && (
            <motion.div key="revealed" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 150, damping: 12 }}
              className="flex flex-col items-center gap-6 w-full">
              <motion.div initial={{ y: -200, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}
                className="flex gap-3">
                {spots.map((spot, i) => (
                  <motion.span key={i}
                    animate={i === correctIndex && flagFlying ? { y: -300, opacity: 0, rotate: 30 } : {}}
              ...
              <motion.img initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }}
                ...
                className="w-56 h-40 object-contain drop-shadow-xl rounded-2xl bg-white p-3 shadow-xl" draggable={false} />
              <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}
